    ...
    mutationFn: async (studentData: CreateStudentData) => {
      ...
      if (!finalData.voting_id) {
        ...
        do {
          ...
          const cleanRollNumber = finalData.roll_number.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(0, 8)
      ...
      if (error) {
        ...
        if (error.code === '23505' && error.message.includes('students_admin_id_roll_number_key')) {
          throw new Error(`A student with roll number "${finalData.roll_number}" already exists. Please use a different roll number.`)
        }
  ...
  const bulkUploadMutation = useMutation({
    mutationFn: async (bulkData: BulkUploadData) => {
      ...
      const studentsWithAdmin = bulkData.students.map(student => ({
        ...
        admin_id: adminId,
        // Auto-generate voting_id if not provided
        voting_id: student.voting_id || `V${student.roll_number.replace(/[^a-zA-Z0-9]/g, '').toUpperCase().slice(0, 8)}${Math.floor(1000 + Math.random() * 9000)}`,
      ...
      if (error) {
        ...
        if (error.code === '23505' && error.message.includes('students_admin_id_roll_number_key')) {
          ...
          throw new Error(`Duplicate roll number found: "${duplicateRollNumber}" already exists. Please check your data and ensure all roll numbers are unique.`)
        }
  ...
  const updateStudentMutation = useMutation({
    mutationFn: async ({ id, ...updateData }: Partial<CreateStudentData> & { id: string }) => {
      ...
      if (error) {
        ...
        // Handle specific constraint violations with user-friendly messages
        if (error.code === '23505' && error.message.includes('students_admin_id_roll_number_key')) {
          throw new Error(`A student with roll number "${updateData.roll_number}" already exists. Please use a different roll number.`)
        }
  ...
  const deleteStudentMutation = useMutation({
    mutationFn: async (id: string) => {
      ...
      const { data: votes } = await supabase
        .from('votes')
        .select('id')
        .eq('student_voting_id', (students || []).find(s => s.id === id)?.voting_id || '')

      if (votes && votes.length > 0) {
        throw new Error('Cannot delete student who has already voted in elections')
      }
      ...
      const { error } = await supabase
        ...
        .delete()
        .eq('id', id)
        .eq('admin_id', user.id) // Ensure user can only delete their own students
